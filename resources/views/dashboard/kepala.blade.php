        ...
        <div class="col-xl-8 col-lg-7">
                    ...
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            ...
                            <div class="h6 mb-0 font-weight-bold text-gray-800">
                                Berdasarkan analisis sistem, terdapat <span class="text-danger">{{ $totalOverload }}</span> pegawai yang mengalami overload beban kerja. Disarankan untuk melakukan redistribusi tugas kepada pegawai dengan beban rendah.
                            </div>
                        </div>
                        <div class="col-auto">
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge badge-success px-2">{{ count(array_filter($pegawaiResult, fn($p) => $p['cluster'] == 'Beban Rendah')) }} Tersedia</span>
                </div>
                    ...
                    <div class="table-responsive" style="max-height: 385px; overflow-y: auto;">
                        <table class="table table-hover mb-0" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                @foreach($pegawaiResult as $p)
                                    @if($p['cluster'] == 'Beban Rendah')
                                        @php $hasLow = true; @endphp
                                        <tr>
                                            <td class="ps-3 fw-bold align-middle">{{ $p['nama'] }}</td>
                                            <td class="align-middle">
                                                <span class="badge badge-light text-dark border">{{ number_format($p['points'], 0) }} Poin</span>
                                            </td>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-sm mb-0" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="ps-3 fw-bold align-middle">{{ $p['nama'] }}</td>
                                    <td class="text-center align-middle">{{ $p['agendas'] }}</td>
                                    <td class="text-center align-middle">
                                        <div class="font-weight-bold">{{ $p['points'] }}</div>
                                    </td>
                                    <td class="text-center align-middle">{{ $p['pending'] }}</td>
                                    <td class="text-center align-middle">
                                        <span class="badge badge-{{ $p['cluster'] == 'Beban Tinggi' ? 'danger' : ($p['cluster'] == 'Beban Ideal' ? 'success' : 'warning') }}">
                                            {{ $p['cluster'] }}
                                        </span>
                                    </td>
                                    <td class="small text-muted italic align-middle">{{ $p['reason'] }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    ...
    var workloadChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: [{{ $counts['Beban Tinggi'] }}, {{ $counts['Beban Ideal'] }}, {{ $counts['Beban Rendah'] }}],
