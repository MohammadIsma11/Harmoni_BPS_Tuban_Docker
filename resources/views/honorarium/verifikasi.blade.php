        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="ps-4">
                            <div class="fw-bold text-dark">{{ $penugasan->mitra->nama_lengkap }}</div>
                            <div class="small text-muted">{{ $penugasan->nama_kegiatan_full }}</div>
                            <small class="badge bg-secondary-subtle text-secondary rounded-pill mt-1">Dibuat oleh: {{ $penugasan->user->nama_lengkap }}</small>
                        </td>
                        <td class="text-center">
                            <div class="fw-bold text-primary">Rp {{ number_format($penugasan->total_honor_tugas, 0, ',', '.') }}</div>
                            <small class="text-muted">Target: {{ Carbon\Carbon::parse($penugasan->tgl_selesai_target)->format('d/m/Y') }}</small>
                        </td>
                        <td>
                            @if($penugasan->file_pendukung)
                                <a href="{{ asset('storage/'.$penugasan->file_pendukung) }}" target="_blank" class="btn btn-outline-info btn-sm rounded-pill">
                        ...
                        <td>
                            ...
                            @endphp
                            <span class="badge bg-{{ $statusClass }} rounded-pill px-3">{{ $penugasan->status_dokumen }}</span>
                        </td>
                        <td class="pe-4 text-end">
                            <form action="{{ route('honorarium.update-status', $penugasan->id) }}" method="POST">
                                @csrf
