...
<div class="matrix-page-wrapper">
        ...
        <div class="col-lg-3 mb-3 mb-lg-0">
            <div class="toolbar-card bg-white shadow-sm rounded-4 p-3 h-100">
                ...
                <div class="d-flex gap-2">
                    <form action="{{ route('monitoring.index') }}" method="GET" id="filterForm" class="flex-grow-1">
                    ...
                    <a href="{{ route('monitoring.index') }}" class="btn btn-danger btn-reset-custom shadow-sm px-3" title="Reset Filter">
        ...
        <div class="col-lg-9">
            <div class="legend-card bg-white shadow-sm rounded-4 p-3 h-100">
                ...
                <div class="d-flex flex-wrap gap-2">
                    ...
                    @foreach($technicalTeams as $team => $color)
                        <div class="legend-item">
                            <span class="inline-dot" style="background: {{ $color }};"></span>
                            <small class="fw-bold">{{ $team }}</small>
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    {{-- Page Title --}}
    <h4 class="fw-bold text-dark mb-4 ms-2"><i class="fas fa-calendar-alt text-primary me-2"></i>Matriks Kegiatan Tahun {{ $year }}</h4>
                ...
                <div class="col">
                    <div class="card month-matrix-card h-100 shadow-sm border-0">
                        <div class="card-header bg-white border-0 pt-4 px-4 pb-0">
                            <h6 class="fw-bold text-dark mb-0 d-flex align-items-center">{{ $mName }}
                                @if($currentMonthFlag) <span class="ms-2 badge bg-primary text-white border-0 small" style="font-size: 0.6rem;">Bulan Ini</span> @endif
                            </h6>
                        </div>
                        <div class="card-body px-4 pb-4">
                            <div class="calendar-matrix-grid mb-3">
                                ...
                                <div class="grid-row days">
                                    @for($i = 0; $i < $weekdayStart; $i++) <div class="cell placeholder"></div> @endfor
                                    @for($day = 1; $day <= $daysCount; $day++)
                                        ...
                                        @endphp
                                        <div class="cell {{ $isTodayDate ? 'active-today' : '' }} {{ $hasActs ? 'has-events' : '' }}"
                                             @if($hasActs) onclick="openDayDetail({{ $mNum }}, {{ $day }}, '{{ $mName }}')" @endif>
                                            <span class="num">{{ $day }}</span>
                                            @if($hasActs)
                                                <div class="event-indicators-wrap">
                                                    @foreach(array_slice($actsList, 0, 3) as $act)
                                                        <span class="e-pill" style="background-color: {{ $act['color'] }}" title="{{ $act['title'] }}"></span>
                                                    @endforeach
                                                </div>
                                            @endif
                                        </div>
                            ...
                            <div class="activities-summary border-top pt-3 mt-auto">
                                <p class="small fw-bold text-dark mb-2">Kegiatan ({{ isset($matrixData[$mNum]) ? count($matrixData[$mNum], COUNT_RECURSIVE) - count($matrixData[$mNum]) : 0 }})</p>
                                @if(isset($matrixData[$mNum]))
                                    <div class="summary-list-scrollable">
                                        @php $shown = 0; @endphp
                                        @foreach($matrixData[$mNum] as $dActs)
                                            @foreach($dActs as $act)
                                                @if($shown < 2)
                                                    <div class="summary-item d-flex align-items-center mb-1"><span class="inline-dot me-2" style="background-color: {{ $act['color'] }}"></span><span class="text-muted small text-truncate">{{ $act['title'] }}</span></div>
                                                @endif
                                                @php $shown++; @endphp
                                            @endforeach
                                        @endforeach
                                        @if($shown > 2) <small class="text-primary fw-bold">+ {{ $shown - 2 }} lainnya</small> @endif
                                    </div>
                                @else <p class="small text-muted mb-0 italic">Tidak ada kegiatan</p> @endif
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
<script>
    window.monitoringData = @json($matrixData);
