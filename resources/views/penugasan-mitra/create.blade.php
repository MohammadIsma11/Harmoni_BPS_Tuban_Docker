...
@section('content')
<form action="{{ route('penugasan-mitra.store') }}" method="POST" enctype="multipart/form-data" id="formPenugasan">
    @csrf
    <div class="row">
                ...
                <div class="d-flex align-items-center mb-4">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-12">
                            ...
                            <input type="hidden" name="kategori_kegiatan" value="Pendataan"> {{-- Defaulting category --}}
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6 text-end">
                            {{-- Space placeholder for removed field --}}
                        </div>
                    </div>
                    ...
                    <div id="volume-inputs-container">
                        {{-- JS will inject volume inputs here for each selected mitra --}}
                    </div>

                    <div class="d-flex justify-content-end gap-2 mt-4">
                        <a href="{{ route('penugasan-mitra.index') }}" class="btn btn-light rounded-pill px-4 fw-bold text-muted">Batal</a>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr class="mitra-row cursor-pointer" 
                                data-name="{{ strtolower($m->nama_lengkap) }}"
                                data-kec="{{ strtolower($m->alamat_kec ?? 'none') }}"
                                data-current-honor="{{ $m->total_honor_month }}"
                                data-max-honor="{{ $m->max_honor_bulanan ?? 3200000 }}">
                                <td class="ps-3">
                                    <div class="fw-bold text-dark small">{{ $m->nama_lengkap }}</div>
                                    <div class="d-flex align-items-center">
                                        <small class="text-muted font-monospace" style="font-size: 0.6rem;">{{ $m->sobat_id }}</small>
                                        <span class="badge {{ $m->total_honor_month >= ($m->max_honor_bulanan ?? 3200000) ? 'bg-danger' : 'bg-primary bg-opacity-10 text-primary' }} rounded-pill ms-2" style="font-size: 0.5rem;">
                                            Rp {{ number_format($m->total_honor_month, 0, ',', '.') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="pe-1">
                                    <input type="number" name="volumes[{{ $m->sobat_id }}]" 
