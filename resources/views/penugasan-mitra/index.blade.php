...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('penugasan-mitra.create') }}" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="ps-4">
                            <div class="fw-bold text-dark">{{ $p->mitra->nama_lengkap }}</div>
                            <small class="text-muted">{{ $p->mitra_id }}</small>
                        </td>
                        <td>
                            <div class="text-dark small fw-bold">{{ $p->nama_kegiatan_full }}</div>
                            @if($p->anggaran && $p->anggaran->team)
                                <span class="badge bg-primary-subtle text-primary small rounded-pill" style="font-size: 0.6rem;">{{ $p->anggaran->team->nama_tim }}</span>
                            @else
                                <span class="badge bg-light text-muted small rounded-pill" style="font-size: 0.6rem;">Input Manual</span>
                            @endif
                        </td>
                        <td class="text-center">
                            <span class="fw-bold text-dark">Rp {{ number_format($p->total_honor_tugas, 0, ',', '.') }}</span>
                        </td>
                        <td class="text-center">
                            <span class="text-muted small">{{ Carbon\Carbon::parse($p->tgl_selesai_target)->format('d M Y') }}</span>
                        </td>
                        <td class="text-center">
                        ...
                        <td class="text-center">
                            <div class="d-flex gap-1 justify-content-center">
                                <a href="{{ route('penugasan-mitra.spk', $p->id) }}" target="_blank" class="btn btn-primary btn-xs py-1 px-3 rounded-pill shadow-sm" style="font-size: 0.65rem;">
                                    <i class="fas fa-print me-1"></i>SPK
                                </a>
                                <a href="{{ route('penugasan-mitra.bast', $p->id) }}" target="_blank" class="btn btn-secondary btn-xs py-1 px-3 rounded-pill shadow-sm" style="font-size: 0.65rem;">
                        ...
                        <td class="pe-4 text-end">
                            ...
                            <div class="d-flex flex-column align-items-end">
                                <span class="badge bg-{{ $docStatusClass }} rounded-pill px-3 mb-2">{{ $p->status_dokumen }}</span>
                                <div class="d-flex gap-1 justify-content-end">
                                    @if($p->status_tugas == 'Progres')
                                        <form action="{{ route('penugasan-mitra.update-status-tugas', $p->id) }}" method="POST">
                                            @csrf
                                    ...
                                    @endif
                                    <form action="{{ route('penugasan-mitra.destroy', $p->id) }}" method="POST" onsubmit="return confirm('Hapus penugasan ini? Tindakan ini tidak dapat dibatalkan.')">
                                        @csrf
                                        @method('DELETE')
