            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                <div class="card-body p-4">
                    <div class="mb-4">
                        ...
                        <h5 class="fw-bold text-dark">{{ $meeting->title }}</h5>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="p-3 bg-light rounded-4 h-100">
                                ...
                                <div class="fw-bold text-dark"><i class="fas fa-map-marker-alt text-danger me-2"></i>{{ $meeting->location }}</div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="p-3 bg-light rounded-4 h-100">
                                ...
                                <div class="fw-bold text-dark">
                                    <i class="far fa-calendar-alt text-primary me-2"></i>{{ \Carbon\Carbon::parse($meeting->event_date)->translatedFormat('d F Y') }}
                                </div>
                    ...
                    <div class="mt-4 pt-4 border-top">
                        ...
                        <div class="d-flex align-items-center p-3 border rounded-4 bg-white shadow-xs">
                            ...
                            <a href="{{ asset('storage/' . $meeting->notulensi_hasil) }}" target="_blank" class="btn btn-success btn-sm rounded-pill px-3">
                            ...
                            @foreach($fotos as $foto)
                                <div class="col-6">
                                    <div class="position-relative img-hover-container rounded-3 overflow-hidden shadow-sm" style="height: 140px;">
                                        <img src="{{ asset('storage/' . $foto) }}" class="w-100 h-100 object-fit-cover" alt="Dokumentasi">
                                        
                                        {{-- Overlay Buttons --}}
                                        <div class="img-overlay d-flex align-items-center justify-content-center gap-2">
                                            <a href="{{ asset('storage/' . $foto) }}" target="_blank" class="btn btn-light btn-sm rounded-circle shadow-sm" title="Lihat Foto">
                                                <i class="fas fa-search-plus text-primary"></i>
                                            </a>
                                            <a href="{{ asset('storage/' . $foto) }}" download class="btn btn-light btn-sm rounded-circle shadow-sm" title="Download">
                    ...
                    <div class="mt-4 pt-4 border-top">
                        ...
                        <div class="d-flex align-items-center">
                            <div class="avatar-sm-table bg-primary text-white me-3" style="width: 45px; height: 45px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-weight: bold;">
                                {{ strtoupper(substr($meeting->assignee->nama_lengkap ?? 'A', 0, 1)) }}
                            </div>
                            <div>
                                <div class="fw-bold text-dark">{{ $meeting->assignee->nama_lengkap ?? 'N/A' }}</div>
                                <div class="small text-muted">{{ $meeting->assignee->role ?? 'Pegawai' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
